...
interface NoteInputProps {
  ...
  onClear: () => void;
  defaultFormat: NoteFormatType;
  isProcessing: boolean;
}

export function NoteInput({ onSubmit, onClear, defaultFormat, isProcessing }: NoteInputProps) {
  ...
  const [noteFormat, setNoteFormat] = useState<NoteFormatType>(defaultFormat);
  ...
  // Update noteFormat when defaultFormat changes
  useEffect(() => {
      ...
      {!isGuided && <div className="space-y-2">
        ...
        <div className="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <div className="flex items-baseline justify-between mb-2">
            <div>
              ...
              <span className="text-sm text-gray-500">
                Using {NOTE_FORMATS[noteFormat].label} Format
              </span>
            </div>
          ...
          <div className="flex items-baseline justify-between mb-4">
            <span className="text-xs text-gray-500">Quick add (+) buttons from sections above will insert text at cursor position</span>
          </div>
          <textarea
            id="content"
            ref={textareaRef}
